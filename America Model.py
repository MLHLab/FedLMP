...
train_data_america = pd.read_csv(io.BytesIO(uploaded['train_data_america.csv']), encoding='unicode_escape')
print(df)
...
test_data_america = pd.read_csv(io.BytesIO(uploaded['test_data_america.csv']), encoding='unicode_escape')
print(df)
...
index_file_path = '/content/drive/MyDrive/Vector/america_vector.index'
...
# Load and run with Central for america Data
...
# Load and run with Europe for America Data
...
# Load and run with Asia for America Data
index = faiss.read_index('/content/drive/MyDrive/asia_vector.index')


# In[ ]:


csv_file_path = '/content/drive/MyDrive/Vector/asia_class.csv'
asia_class = pd.read_csv(csv_file_path)
print(asia_class)
...
for i in range(len(test_data)):
    min_distance_index = indices[i][0]  # Index of nearest neighbor with smallest distance
    predicted_dis_hier1 = asia_class.iloc[min_distance_index]['disease_hierarch1']
    predicted_dis_hier2 = asia_class.iloc[min_distance_index]['disease_hierarch2']
    actual_dis_hier1 = test_data.iloc[i]['disease_hierarch1']
    actual_dis_hier2 = test_data.iloc[i]['disease_hierarch2']
    test_index = i
    distance = distances[i]
